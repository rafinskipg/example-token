...
describe("distributor", function () {
  beforeEach(async () => {
    ...
    exampleToken = await ERC20.deploy(ownerAmount, owner.address);
    ...
    await exampleToken
      .connect(owner)
      .transfer(distributor.address, parseEther("9000000"));
  });
  ...
  it("Should distribute 1000 tokens per address to 10 addresses", async () => {
    const addresses = [
      ...
      user2.address,
      user3.address,
      ...rest.map((x) => x.address),
    ];
    await distributor.distribute(parseEther("1000"), addresses);
    for (let i = 0; i < addresses.length; i++) {
      expect(await exampleToken.balanceOf(addresses[i])).to.equal(
        parseEther("1000")
      );
    }
  });
  ...
  it("A non owner is not allowed to distribute", async () => {
    await expect(
      distributor.connect(user1).distribute(parseEther("1000"), [user1.address])
  ...
  it("allows to withdraw the tokens", async () => {
    ...
    expect(await exampleToken.balanceOf(owner.address)).to.equal(
      parseEther("10000000")
    );
  });
});
